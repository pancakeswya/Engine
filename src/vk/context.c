...
static const uint32_t kDeviceExtensionsCount = sizeof(kDeviceExtensions) / sizeof(const char*);
static const VkDynamicState kPipelineDynamicStates[] = {
    VK_DYNAMIC_STATE_VIEWPORT,
    VK_DYNAMIC_STATE_SCISSOR
};
static const uint32_t kPipelineDynamicStatesCount = sizeof(kPipelineDynamicStates) / sizeof(VkDynamicState);
#ifdef DEBUG
...
static Error messengerCreate(
    ...
    const VkResult res = createMessengerFn(instance, create_info, NULL, messenger);
    if (res != VK_SUCCESS) {
...
static Error layersSupport(void) {
    ...
    VkLayerProperties* available_layers = (VkLayerProperties*)malloc(
        vk_layer_count * sizeof(VkLayerProperties)
    );
    ...
    vk_res = vkEnumerateInstanceLayerProperties(&vk_layer_count, available_layers);
    if (vk_res != VK_SUCCESS) {
...
static inline uint32_t clamp(const uint32_t val, const uint32_t min, const uint32_t max) {
    if (val < min)
        return min;
    if (val > max)
        return max;
...
static Error deviceExtensionsSupport(const VkPhysicalDevice device, bool* support) {
    *support = true;
    uint32_t extension_count = 0;
    VkResult vk_res = vkEnumerateDeviceExtensionProperties(device, NULL, &extension_count, NULL);
    if (vk_res != VK_SUCCESS) {
        return (Error){vk_res, kErrorTypeVulkan};
    }
    ...
    vk_res = vkEnumerateDeviceExtensionProperties(device, NULL, &extension_count, available_extensions);
    if (vk_res != VK_SUCCESS) {
    ...
    for(uint32_t i = 0; i < kDeviceExtensionsCount; ++i) {
        ...
        for(uint32_t j = 0; j < extension_count; ++j) {
            if (strcmp(kDeviceExtensions[i], available_extensions[j].extensionName) == 0) {
                found = true;
                break;
            }
        }
...
static Error SurfaceSupport(
  const VkPhysicalDevice device,
  const VkSurfaceKHR surface,
  SurfaceSupportDetails* details
) {
    VkSurfaceCapabilitiesKHR capabilities = {0};
    VkResult vk_res = vkGetPhysicalDeviceSurfaceCapabilitiesKHR(
        device, surface, &capabilities);
    if (vk_res != VK_SUCCESS) {
        return (Error){vk_res, kErrorTypeVulkan};
    }
    uint32_t formats_count = 0;
    vk_res = vkGetPhysicalDeviceSurfaceFormatsKHR(device, surface, &formats_count, NULL);
    if (vk_res != VK_SUCCESS) {
        return (Error){vk_res, kErrorTypeVulkan};
    }
    ...
    if (formats_count != 0) {
        ...
        vk_res = vkGetPhysicalDeviceSurfaceFormatsKHR(device, surface, &formats_count, formats);
        if (vk_res != VK_SUCCESS) {
            free(formats);
            return (Error){vk_res, kErrorTypeVulkan};
        }
    ...
    vk_res = vkGetPhysicalDeviceSurfacePresentModesKHR(device, surface, &present_modes_count, NULL);
    if (vk_res != VK_SUCCESS) {
        free(formats);
        return (Error){vk_res, kErrorTypeVulkan};
    }
...
static Error findQueueFamilyIndices(
    const VkPhysicalDevice device,
    const VkSurfaceKHR surface,
    ...
    *found = false;
    ...
    for (uint32_t i = 0; i < families_count; ++i) {
        ...
        const VkResult vk_res = vkGetPhysicalDeviceSurfaceSupportKHR(device, i, surface, &present_support);
        if (vk_res != VK_SUCCESS) {
            free(families);
            return (Error){vk_res, kErrorTypeVulkan};
        }
        ...
        if (graphics_found &&
            ...
            if (ext_support) {
                ...
                if (details->formats && details->present_modes) {
                    *found = true;
                    *indices = (QueueFamilyIndices) {.present = present, .graphics = graphics };
                    break;
                }
            }
        }
...
static Error createPhysicalDevice(
    const VkInstance instance,
    const VkSurfaceKHR surface,
    ...
    for (uint32_t i = 0; i < device_count; ++i) {
        bool found = false;
        const Error err = findQueueFamilyIndices(devices[i], surface, indices, details, &found);
        if (!ErrorEqual(err, kErrorSuccess)) {
            free(devices);
            return err;
        }
...
static Error createLogicalDevice(
    const VkPhysicalDevice physical_device,
    const QueueFamilyIndices* indices,
    VkDevice* logical_device
) {
    const uint32_t family_ids[] = { indices->graphics, indices->present };
    const size_t unique_family_count = (indices->graphics == indices->present) ? 1 : 2;
    VkDeviceQueueCreateInfo* queue_create_infos =
        (VkDeviceQueueCreateInfo*)malloc(unique_family_count * sizeof(VkDeviceQueueCreateInfo));
    ...
    const VkResult vk_res = vkCreateDevice(physical_device, &create_info, NULL, logical_device);
...
static VkSurfaceFormatKHR chooseSwapSurfaceFormat(const VkSurfaceFormatKHR available_formats[], const uint32_t count) {
...
static VkPresentModeKHR chooseSwapPresentMode(const VkPresentModeKHR available_present_modes[], const uint32_t count) {
    for (uint32_t i = 0; i < count; ++i) {
...
static Error createSwapchain(
    ...
    const VkResult vk_res = vkCreateSwapchainKHR(logical_device, &create_info, NULL, swapchain);
    if (vk_res != VK_SUCCESS) {
        return (Error){vk_res, kErrorTypeVulkan};
    }
    *extent_ptr = extent;
...
static Error createImages(
    ...
    VkResult vk_res = vkGetSwapchainImagesKHR(logical_device, swapchain, &image_count, NULL);
    if (vk_res != VK_SUCCESS) {
        return (Error){vk_res, kErrorTypeVulkan};
    }
...
static Error createRenderPass(
    ...
    const VkResult vk_res = vkCreateRenderPass(logical_device, &render_pass_info, NULL, render_pass);
    if (vk_res != VK_SUCCESS) {
        return (Error){vk_res, kErrorTypeVulkan};
    }
    return kErrorSuccess;
...
static Error createPipelineLayout(const VkDevice logical_device, VkPipelineLayout* pipeline_layout) {
    const VkPipelineLayoutCreateInfo pipeline_layout_info = {
        .sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO,
        .setLayoutCount = 0,
        .pushConstantRangeCount = 0
    };
    const VkResult vk_res = vkCreatePipelineLayout(logical_device, &pipeline_layout_info, NULL, pipeline_layout);
    if (vk_res!= VK_SUCCESS) {
        return (Error){vk_res, kErrorTypeVulkan};
    }
    return kErrorSuccess;
...
static Error createShaderModule(const VkDevice logical_device, const char* path, VkShaderModule* module) {
    size_t read = 0;
    char* code = NULL;
    ...
    VkResult vk_res = vkCreateShaderModule(logical_device, &create_info, NULL, module);
    free(code);
    if (vk_res != VK_SUCCESS) {
        return (Error){vk_res, kErrorTypeVulkan};
    }
...
static Error createPipeline(
    ...
    Error err = createShaderModule(logical_device, "shaders/vert.spv", &vert_shader_module);
    if (!ErrorEqual(err, kErrorSuccess)) {
        return err;
    }
    err = createShaderModule(logical_device, "shaders/frag.spv", &frag_shader_module);
    if (!ErrorEqual(err, kErrorSuccess)) {
    ...
    const uint32_t shader_stages_count = sizeof(shader_stages) / sizeof(VkPipelineShaderStageCreateInfo);
    ...
    VkResult vk_res = vkCreateGraphicsPipelines(logical_device, VK_NULL_HANDLE, 1, &pipeline_info, NULL, pipeline);
    vkDestroyShaderModule(logical_device, vert_shader_module, NULL);
...
static Error createCmdPool(
    ...
    const VkResult vk_res = vkCreateCommandPool(logical_device, &create_info, NULL, cmd_pool);
    if (vk_res != VK_SUCCESS) {
        return (Error){vk_res, kErrorTypeVulkan};
    }
    return kErrorSuccess;
...
static Error createCmdBuffers(
    ...
    VkCommandBuffer* cmd_buffers = (VkCommandBuffer*)malloc(
      count * sizeof(VkCommandBuffer)
    );
    const VkResult vk_res = vkAllocateCommandBuffers(logical_device, &alloc_info, cmd_buffers);
    if (vk_res != VK_SUCCESS) {
        free(cmd_buffers);
        return (Error){vk_res, kErrorTypeVulkan};
    }
...
static Error createImageView(
    ...
    const VkResult vk_res = vkCreateImageView(logical_device, &create_info, NULL, view);
    if (vk_res != VK_SUCCESS) {
        return (Error){vk_res, kErrorTypeVulkan};
    }
    return kErrorSuccess;
...
static Error createImageViews(
    ...
    for(uint32_t i = 0; i < image_count; ++i) {
        const Error err = createImageView(
            logical_device, images[i],
            format, &image_views[i]
        );
        if (!ErrorEqual(err, kErrorSuccess)) {
            free(image_views);
            return err;
...
static Error createFramebuffer(
    const VkDevice logical_device,
    const VkRenderPass render_pass,
    ...
    const VkResult vk_res = vkCreateFramebuffer(logical_device, &create_info, NULL, framebuffer);
    if (vk_res != VK_SUCCESS) {
        return (Error){vk_res, kErrorTypeVulkan};
    }
    return kErrorSuccess;
...
static Error createFramebuffers(
    const VkDevice logical_device,
    const VkRenderPass render_pass,
...
static Error createSemaphore(
    ...
    const VkResult vk_res = vkCreateSemaphore(logical_device, &create_info, NULL, semaphore);
    if (vk_res != VK_SUCCESS) {
        return (Error){vk_res, kErrorTypeVulkan};
    }
    return kErrorSuccess;
...
static Error createFence(
    ...
    const VkResult vk_res = vkCreateFence(logical_device, &create_info, NULL, fence);
    if (vk_res != VK_SUCCESS) {
        return (Error){vk_res, kErrorTypeVulkan};
    }
    return kErrorSuccess;
...
Error VulkanContextCreate(VulkanContext* context, GLFWwindow* window) {
...
#ifdef DEBUG
    err = messengerCreate(context->instance, &messenger_create_info, &context->messenger);
    if (!ErrorEqual(err, kErrorSuccess)) {
        VulkanContextDestroy(context);
        return err;
    }
#endif
    VkResult vk_res = glfwCreateWindowSurface(context->instance, window, NULL, &context->surface);
    if (vk_res != VK_SUCCESS) {
        VulkanContextDestroy(context);
        return (Error){vk_res, kErrorTypeVulkan};
    }
    ...
    err = createPhysicalDevice(context->instance, context->surface, &context->physical_device, &indices, &details);
    if (!ErrorEqual(err, kErrorSuccess)) {
        VulkanContextDestroy(context);
        return err;
    }
    err = createLogicalDevice(context->physical_device, &indices, &context->logical_device);
    if (!ErrorEqual(err, kErrorSuccess)) {
        freeSurfaceSupportDetails(&details);
        VulkanContextDestroy(context);
        return err;
    }
    vkGetDeviceQueue(context->logical_device, indices.graphics, 0, &context->graphics_queue);
    vkGetDeviceQueue(context->logical_device, indices.present, 0, &context->present_queue);
    err = createSwapchain(
        window, context->logical_device, context->surface,
    ...
    err = createImages(
        context->logical_device, context->swapchain,
    ...
    err = createImageViews(
        context->logical_device, context->images,
    ...
    err = createRenderPass(context->logical_device, context->format, &context->render_pass);
    if (!ErrorEqual(err, kErrorSuccess)) {
        VulkanContextDestroy(context);
        return err;
    }
    err = createPipelineLayout(context->logical_device, &context->pipeline_layout);
    if (!ErrorEqual(err, kErrorSuccess)) {
        VulkanContextDestroy(context);
        return err;
    }
    err = createPipeline(
        context->logical_device, context->pipeline_layout,
    ...
    err = createFramebuffers(
        context->logical_device, context->render_pass,
        context->image_views, context->image_view_count,
    ...
    err = createCmdBuffers(
        context->logical_device, context->cmd_pool,
        1, &context->cmd_buffers
    );
    if (!ErrorEqual(err, kErrorSuccess)) {
        VulkanContextDestroy(context);
        return err;
...
void VulkanContextDestroy(const VulkanContext* context) {
    ...
    if (context->render_semaphore != VK_NULL_HANDLE) {
        vkDestroySemaphore(context->logical_device, context->render_semaphore, NULL);
    }
    ...
    if (context->cmd_buffers != NULL) {
        vkFreeCommandBuffers(context->logical_device,  context->cmd_pool, 1, context->cmd_buffers);
    ...
    if (context->framebuffers != NULL) {
        for(size_t i = 0; i < context->framebuffer_count; ++i) {
            vkDestroyFramebuffer(context->logical_device, context->framebuffers[i], NULL);
        }
