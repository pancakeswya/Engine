...
Renderer::Renderer(window::gl::Window& window)
    : window_(window),
...
void Renderer::LoadModel(const std::string& path) {
  ...
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(Index) * data.indices.size(),  nullptr, GL_STATIC_DRAW);

  auto indices = static_cast<Index*>(glMapBuffer(GL_ELEMENT_ARRAY_BUFFER, GL_WRITE_ONLY));

  PointerHandle vbo(buffer_deleter);
  glGenBuffers(1, vbo.Ptr());
  glBindBuffer(GL_ARRAY_BUFFER, vbo.Value());
  glBufferData(GL_ARRAY_BUFFER, sizeof(Vertex) * data.indices.size(), nullptr, GL_STATIC_DRAW);
  auto vertices = static_cast<Vertex*>(glMapBuffer(GL_ARRAY_BUFFER, GL_WRITE_ONLY));

  data_util::RemoveDuplicates(data, vertices, indices);
...
void Renderer::RenderFrame() {
  ...
  const Uniforms& uniforms = model_.GetUniforms();
