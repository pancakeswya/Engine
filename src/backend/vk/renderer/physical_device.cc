...
bool PhysicalDevice::GetFormatFeatureSupported(const VkFormat format, const VkFormatFeatureFlagBits feature) const {
...
bool PhysicalDevice::GetExtensionsSupport(const std::vector<const char*>& extensions) const {
...
VkBool32 PhysicalDevice::GetSurfaceSupported(VkSurfaceKHR surface, const uint32_t queue_family_idx) const {
